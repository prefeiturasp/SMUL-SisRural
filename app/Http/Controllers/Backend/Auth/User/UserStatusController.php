...
class UserStatusController extends Controller
{
    ...
    public function mark(ManageUserRequest $request, User $user, $status)
    {
        ...
        if (strpos(\URL::previous(), "/roles") !== FALSE) {
            $redirect = route('admin.auth.user.roles.index', compact('user'));
        }

        return redirect($redirect)->withFlashSuccess(__('alerts.backend.users.updated'));
    }
